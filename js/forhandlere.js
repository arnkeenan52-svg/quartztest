...
document.addEventListener('DOMContentLoaded', () => {
  ...
  FORHANDLERE.forEach(f => {
    ...
    marker.bindPopup(`
      ...
      ${f.phone ? `<div><a href="tel:${escapeHTML(f.phone.replace(/\s/g, ''))}">${escapeHTML(f.phone)}</a></div>` : ''}
      ${f.url ? `<a href="${escapeHTML(f.url)}" target="_blank" rel="noopener">Besøg hjemmeside →</a>` : ''}
    `);
