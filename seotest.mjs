...
t('forside: ét samlet @graph', g.length===4, g.map(n=>n['@type']).join(' / '));
...
// ── Undersider ────────────────────────────────────────────────────────────
const UNDER = { 'shop.html':['Shop','shop'], 'om.html':['Om os','om'], 'forhandlere.html':['Forhandlere','forhandlere'] };
...
for (const [fil,[navn,sti]] of Object.entries(UNDER)) {
  ...
  const wp = ld(s).flatMap(x=>x['@graph']||[x]).find(n=>n['@type']==='WebPage');
...
try {
  ...
  t('produkt: AggregateOffer ved flere vægte', r.prod.offers['@type']==='AggregateOffer' && +r.prod.offers.lowPrice < +r.prod.offers.highPrice, `${r.prod.offers.lowPrice}–${r.prod.offers.highPrice}`);
  t('produkt: sælger = organisationen', r.prod.offers.seller['@id']===org['@id']);
  t('produkt: sku og kategori', r.prod.sku==='rod-hvede-type70' && r.prod.category==='Mel');
  t('produkt: brødkrumme Hjem › Shop › vare', r.krum.itemListElement.length===3 && r.krum.itemListElement[1].name==='Shop' && r.krum.itemListElement[2].item===r.canon);
